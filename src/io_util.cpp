...
// "flag" "identifier"
std::map<std::string, std::string> jax::cl_args = {
...
void jax::toLower(std::string &str){
  std::locale loc;
...
bool jax::fileExists(std::string fileName){
    std::ifstream infile(fileName);
    return infile.good();
}
bool jax::directoryExists(std::string dirPath){
 ...
 */
std::string jax::getFileExtension(std::string path){
 ...
 * @return string
 */
std::string jax::getFolderFromFilePath(std::string path){
 ...
 */
std::string jax::getFileFromFilePath(std::string path){
  return path.substr(path.find_last_of("\\/") + 1);
}

void jax::getImagePaths(std::string dirPath, std::vector<std::string> &imagePaths){
  DIR* dir;
...
std::vector<std::string> jax::findFiles(std::string path){
...
jax::img_dir_arg::img_dir_arg(char* path){
...
jax::flt_arg::flt_arg(char* val){
  this->val = std::stof(val);
}

jax::int_arg::int_arg(char* val){
 ...
 * @param
 */
std::map<std::string, jax::arg*> jax::parseArgs(int numArgs, char* args[]){
  if(numArgs < 3){
...
unsigned char* jax::getPixelArray(unsigned char** &row_pointers, const unsigned int &width, const unsigned int &height, const int numValues){
  if(numValues == 0){
...
unsigned char* jax::readPNG(const char* filePath, unsigned int &height, unsigned int &width, unsigned int& colorDepth){
...
void jax::writePNG(const char* filePath, unsigned char* image, const unsigned int &colorDepth, const unsigned int &width, const unsigned int &height){
  /* create file */
...
unsigned char* jax::readTIFF(const char* filePath, unsigned int &height, unsigned int &width, unsigned int &colorDepth){
...
void jax::writeTIFF(const char* filePath, unsigned char* image, const unsigned int &colorDepth, const unsigned int &width, const unsigned int &height){
...
unsigned char* jax::readJPEG(const char* filePath, unsigned int &height, unsigned int &width, unsigned int &colorDepth){
...
void jax::writeJPEG(const char* filePath, unsigned char* image, const unsigned int &colorDepth, const unsigned int &width, const unsigned int &height){
...
unsigned char* jax::readImage(const char *filePath, unsigned int &height, unsigned int &width, unsigned int &colorDepth){
 ...
 */
void jax::writeCSV(float* values, int num, std::string filename){
 ...
 *
 */
bool jax::readImageMeta(std::string imgpath, bcpFormat & out)
{
...
void jax::writePLY(const char* filePath, Unity<float3>* points, bool binary){
